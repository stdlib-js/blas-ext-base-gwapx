...
tape( 'the function has an arity of 6', function test( t ) {
	t.strictEqual( gwapx.length, 6, 'has expected arity' );
	t.end();
});
...
tape( 'the function adds a scalar constant to each element in a strided array `x` and assigns the results to elements in a strided array `w` (accessors)', function test( t ) {
	...
	gwapx( x.length, 5.0, toAccessorArray( x ), 1, toAccessorArray( w ), 1 );
	t.deepEqual( w, expected, 'returns expected value' );
...
tape( 'the function returns a reference to the output array', function test( t ) {
	...
	w = zeros( x.length );
	out = gwapx( x.length, 3.0, x, 1, w, 1 );
...
tape( 'the function supports an `x` stride (accessors)', function test( t ) {
	...
	gwapx( 3, 5.0, toAccessorArray( x ), 2, toAccessorArray( w ), 1 );
	t.deepEqual( w, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports a `w` stride (accessors)', function test( t ) {
	...
	gwapx( 3, 5.0, toAccessorArray( x ), 1, toAccessorArray( w ), 2 );
	t.deepEqual( w, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports negative strides (accessors)', function test( t ) {
	...
	gwapx( 3, 5.0, toAccessorArray( x ), -2, toAccessorArray( w ), -2 );
	t.deepEqual( w, expected, 'returns expected value' );
	t.end();
});
